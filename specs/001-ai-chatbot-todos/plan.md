...
Implementation of an AI-powered chatbot interface for managing todos using natural language. The system will integrate OpenAI Agents SDK (ChatKit UI) configured with the Gemini-compatible base_url and API key to drive natural-language task management, while delegating all task mutations to MCP tools in a stateless architecture. The backend will remain stateless with all conversation and task state persisted in Neon PostgreSQL. The frontend will use ChatKit UI for natural language interaction with the AI agent.

**Main Focus**: Use the OpenAI Agents SDK (ChatKit UI) configured with the Gemini-compatible base_url and API key to drive natural-language task management, while delegating all task mutations to MCP tools in a stateless architecture.
